...
- `bridge-then-continue`: answer or acknowledge the customer briefly, then keep the call open for a low-pressure next step or slower lookup.
...
Use `bridge-then-continue` when the agent should answer or acknowledge first and continue only with a low-pressure next step, or when slower lookup should not leave the customer in silence.

Good fit:

- direct pricing, written-info, stakeholder-review, or partner-review turns where the customer has not refused and the continuation is optional
...
```text
Customer: What does this cost?
Agent: The starter plan is 29 per user per month, and I can send the exact terms in writing. If useful, I can briefly explain what is included before I send them. No payment or commitment on this call.
call_control: bridge-then-continue
