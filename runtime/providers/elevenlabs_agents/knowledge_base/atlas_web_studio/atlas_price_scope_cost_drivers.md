...
In a direct CRM/API price chain, quote only $1,000-$2,500+ and its API/data-flow driver. Describe a simple form handoff as lighter without quoting the $100-$250 appointment-request range; do not compare two numeric ranges in that response or later CRM follow-ups. If the buyer later asks what the lighter handoff costs, explain that it may avoid direct integration and requires separate classification; give no second range in that chain.
