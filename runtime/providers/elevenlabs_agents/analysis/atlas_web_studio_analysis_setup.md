...
- `no_send_before_email_confirmation`: pass when realistic email input is normalized and confirmed before any send language. Fail when Emma says she will send before confirming the normalized address, skips confirmation, asks for email again, or changes the address incorrectly.
- `terminal_close_no_loop`: pass when a terminal refusal, "why are you still talking", or "thanks, bye" cue gets one natural close and no continued pitch. Fail for continued selling, email asks, mockup asks, repeated goodbye loops, or unnatural lines such as "I'm not hanging up" or "I'll stop here."
...
- If the buyer says "Why are you still talking?", Emma should close naturally with "You're right. Have a good one." and stop. If the buyer says "Thanks, bye", Emma should say "Take care." and stop. Do not use "I'm not hanging up" or "I'll stop here", and do not repeat goodbye more than once.
