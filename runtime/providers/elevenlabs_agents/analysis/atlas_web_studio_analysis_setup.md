...
- `cost_driver_expertise`: pass when cost answers give a price ballpark after repeated ask by the first or second direct price ask when campaign facts allow one, then explain real complexity such as custom copy, pages, service-area pages, workflows, integrations, migration, SEO/content work, technical setup, or custom design. Fail if buyer asks price twice and Emma still gives no ballpark, gives only "it depends", repeats free mockup/no obligation instead of answering cost, or reaches max turns because Emma avoided price.
...
- `no_scripted_example_echo`: pass when examples are varied naturally. Fail for canned phrase echo, residue loop, or AI monologue after clarification.
...
- `natural_spoken_quality`: pass when responses are short, conversational, concrete, use a casual transition on scripted pitch turns when natural, and avoid robotic phrases. Fail for AI monologue, long unrequested feature dump, missing casual transition on scripted pitch turns, or brochure/support-script wording.
...
- Human Phone Call Standard: fail for AI monologue, unrequested feature dumps, brochure/support-script wording, or missing casual transition on scripted pitch turns.
...
- Price ballpark after repeated ask: fail if buyer asks price twice and Emma still gives no ballpark, gives only "it depends", repeats free mockup/no obligation instead of answering cost, or reaches max turns because Emma avoided price.
