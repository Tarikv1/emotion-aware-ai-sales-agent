...
- `runtime/README.md`: current boundary map for files that affect runtime behavior.
...
| `runtime/` | Runtime-affecting source modules, runtime-facing Markdown, runtime prompts, campaign examples, voice config example, persistence schema, manifest, and move boundary. |
...
| `scripts/` | Compatibility wrappers for runtime modules plus local runners, validators, setup checks, and safety guards. |
...
- Runtime boundary: `runtime/runtime_manifest.json` maps the runtime-affecting files now housed under `runtime/`, while checkpoint runners, generated evidence, temporary files, and private data stay outside it.
- Voice/runtime delivery: canonical runtime/provider boundaries live under `runtime/`; checkpoint evidence remains in `docs/product/VOICE_*`, `docs/product/RESP_*`, related `scripts/voice_*`, `scripts/runtime_*`, and `scripts/run_resp_*`.
- Thesis traceability: `docs/thesis/METHODOLOGY_LOG.md`, `docs/thesis/DECISION_LOG.md`, `docs/thesis/THESIS_REFERENCE_REGISTRY.md`.

## Before Moving Runtime Files Again Or Removing Wrappers

Many validators point at exact paths. Start with `runtime/runtime_manifest.json` before moving another runtime group or deleting a legacy wrapper. If a move/removal is still useful, update:
