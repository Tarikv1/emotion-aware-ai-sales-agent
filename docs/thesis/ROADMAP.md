...
- [ ] Current: run `PROD-041-conditional-simulation-review` on the repaired `PROD-041A-conditional-scenario-diversity-expansion` traces that now come from concrete scenario frames.
...
- [x] `PROD-041A` conditional scenario diversity expansion repair, which keeps the same `40` scenarios (`24` B2B, `16` B2C) but adds concrete scenario frame mining before dialogue generation. It now writes `concrete_scenario_frames.json`, maps one frame to each scenario label exactly once, and generates dialogue from frame fields (`customer_role`, `real_world_context`, `practical_trigger`, realistic objections, realistic goals, and realistic next steps) instead of direct concern-text templating. The repaired traces keep hard failures, payment collection, unsupported claims, and leakage at `0`, remain offline/deterministic/no-LLM, and keep the next checkpoint as `PROD-041-conditional-simulation-review`.
