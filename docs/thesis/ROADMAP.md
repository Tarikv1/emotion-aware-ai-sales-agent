...
- [ ] Current: run `PROD-041-conditional-simulation-review` on the repaired `PROD-041A-conditional-scenario-diversity-expansion` traces that now come from leakage-safe scenario recipes and concrete fictional frames.
...
- [x] `PROD-041A` conditional scenario diversity expansion repair, which keeps the same `40` scenarios (`24` B2B, `16` B2C) but adds `scenario_recipes.json` before concrete frame generation and `spoken_trace_authoring` before trace output. Recipes store generalized call-center structures and abstract pattern IDs only; `concrete_scenario_frames.json` then invents original fictional contexts and maps one frame to each scenario label exactly once. The spoken authoring layer uses frame facts as semantic inputs only and selects scenario-specific natural-language call scripts instead of injecting `practical_trigger`, `first_customer_objection`, `realistic_next_step`, or `spoken_reason` strings into dialogue. The repaired traces keep hard failures, payment collection, unsupported claims, and leakage at `0`, remain offline/deterministic/no-LLM, and keep the next checkpoint as `PROD-041-conditional-simulation-review`.
