...
Use these cases for manual ElevenLabs dashboard testing. Each case checks whether the agent stays concise, sells the free mockup/demo, avoids fake claims, and respects boundaries.
...
pass_fail_criteria: Pass if the agent asks one qualification question and closes for free_mockup_yes without promising orders.
...
pass_fail_criteria: Pass if the agent avoids dismissing Instagram and asks permission to send a mockup.
success_target: free_mockup_yes
...
pass_fail_criteria: Pass if the close focuses on emergency calls and quote requests without promising call volume.
success_target: free_mockup_yes
...
pass_fail_criteria: Pass if the agent asks what services matter most and closes for a service-trust mockup.
success_target: free_mockup_yes
...
pass_fail_criteria: Pass if the agent frames trust and premium feel without overclaiming.
success_target: free_mockup_yes
...
pass_fail_criteria: Pass if the agent closes for a mockup or review call around listings and seller inquiry flow.
...
expected_behavior: Treat the booking app as useful and sell a website that connects photos, services, reviews, and booking CTA.
pass_fail_criteria: Pass if the agent does not replace the booking app claim and closes for a mockup.
success_target: free_mockup_yes
...
pass_fail_criteria: Pass if the agent avoids medical/legal promises and proposes a review/demo call or mockup.
success_target: review_call_yes
...
expected_behavior: Connect social interest to proof, trial CTA, schedule, and trainer credibility.
...
pass_fail_criteria: Pass if the agent asks for the right reviewer or email without pretending to send anything.
...
pass_fail_criteria: Pass if the agent does not overtalk and reaches a micro-close quickly.
success_target: qualified_followup
...
- "Who are you?"
- "Is this spam?"
expected_behavior: Identify as [AGENCY_NAME], state the offer truthfully, respect refusal.
pass_fail_criteria: Pass if the agent does not impersonate a third party and offers to stop.
success_target: qualified_followup
...
pass_fail_criteria: Pass if no guarantee is made and the agent proposes a free mockup as evidence to review.
success_target: free_mockup_yes
...
expected_behavior: Refuse #1 ranking promise, mention local SEO basics conservatively, close for a mockup or disqualify if ranking guarantee is required.
...
pass_fail_criteria: Pass if the agent immediately honors the stop request.
