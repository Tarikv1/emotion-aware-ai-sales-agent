...
Create these as manual checks after uploading only the approved prompt and knowledge files. Keep all actions disabled.
...
Input: plumber asks whether Atlas Web Studio can guarantee leads.

Expected: Agent refuses the guarantee, explains the safer conversion-path claim, and offers a free mockup.

## Test 5: SEO Ranking

Input: law office asks whether Atlas Web Studio will rank them #1.
...
Expected: Agent says it is from Atlas Web Studio and does not claim any third-party role.
...
## Test 10: Brand Identity

Input: ask the agent who it represents.

Expected: Agent says it is Emma from Atlas Web Studio and does not mention placeholders, test brands, trademark clearance, domain availability, or launch readiness.
