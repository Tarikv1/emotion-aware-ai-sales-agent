...
CASE_FILE = ROOT / "research" / "experiments" / "cases" / "prod-099-english-recommendation-roleplay-narrow-policy-probe.json"

REQUIRED_FILES = {
    "module": ROOT / "scripts" / "prod_099_english_recommendation_roleplay_narrow_policy_probe.py",
    "runner": ROOT / "scripts" / "run_prod_099_english_recommendation_roleplay_narrow_policy_probe.py",
    "validator": ROOT / "scripts" / "validate_prod_099_english_recommendation_roleplay_narrow_policy_probe.py",
    "source_validator": ROOT / "scripts" / "validate_prod_098_english_recommendation_roleplay_review_import.py",
    "doc": ROOT / "docs" / "product" / "PROD_099_ENGLISH_RECOMMENDATION_ROLEPLAY_NARROW_POLICY_PROBE.md",
...
SOURCE_FILES = {
    ...
    "source_candidates": ROOT / "research" / "experiments" / "generated" / SOURCE_CHECKPOINT_ID / "approved_recommendation_roleplay_candidate_packet.json",
    "source_readiness": ROOT / "research" / "experiments" / "generated" / SOURCE_CHECKPOINT_ID / "narrow_policy_probe_readiness.json",
}
...
GENERATED_FILES = {
    ...
    "recommendation_roleplay_probe_case_matrix": OUT_DIR / "recommendation_roleplay_probe_case_matrix.json",
...
def validate_sources() -> None:
    ...
    candidates = read_json(SOURCE_FILES["source_candidates"])
    readiness = read_json(SOURCE_FILES["source_readiness"])
...
def validate_case_file() -> None:
    ...
    assert_condition(payload["scope"] == "english_recommendation_roleplay_narrow_policy_probe", payload)
...
def validate_payloads() -> None:
    ...
    matrix = read_json(GENERATED_FILES["recommendation_roleplay_probe_case_matrix"])
    ...
    assert_condition(summary["recommendation_roleplay_probe_passed"] is True, summary)
    assert_condition(summary["selected_source_slice"] == "recommendation_roleplay_boundary", summary)
    assert_condition(summary["positive_case_count"] >= 7, summary)
    assert_condition(summary["control_case_count"] >= 10, summary)
    assert_condition(summary["failed_policy_case_count"] == 0, summary)
    assert_condition(summary["current_runtime_gap_count"] == 7, summary)
    ...
    assert_condition(constraints["selected_source_slice"] == "recommendation_roleplay_boundary", constraints)
    ...
    assert_condition(matrix["positive_case_count"] >= 7, matrix)
    assert_condition(matrix["control_case_count"] >= 10, matrix)
...
def validate_docs() -> None:
    ...
    for marker in [
        "prod-099",
        "english recommendation roleplay narrow policy probe",
