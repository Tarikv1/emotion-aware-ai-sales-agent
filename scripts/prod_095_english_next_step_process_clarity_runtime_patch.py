...
CHECKPOINT_NAME = "English Next-Step Process Clarity Runtime Patch"
...
def load_source() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if source_result["validation"]["passed"] is not True:
        raise RuntimeError("PROD-094 must pass before PROD-095.")
    if source_result["summary"]["recommended_next_checkpoint"] != CHECKPOINT_ID:
        raise RuntimeError("PROD-094 must recommend PROD-095.")
    if gap_analysis["runtime_gap_count"] != 1:
        raise RuntimeError("PROD-094 must identify exactly one runtime gap.")
...
def build_patch_summary(gap_analysis: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "added_sales_difficulty": "next-step-process-clarity",
        "added_process_signals": ["what happens after", "next step", "after this call", "move forward", "register after", "what happens next"],
        "blocked_boundary_signals": ["payment", "card", "pay now", "sign me up", "sign up", "contract", "provider", "reimbursement", "coverage", "what would you do", "my position", "supposed to decide"],
        "email_link_flag_required": True,
...
def render_report(summary: dict[str, Any], patch: dict[str, Any]) -> str:
    lines = [
        ...
        "## Patch Summary",
        "",
        f"- Added sales difficulty: `{patch['added_sales_difficulty']}`",
        f"- Email link flag required: `{str(patch['email_link_flag_required']).lower()}`",
        f"- Candidate response: `{patch['candidate_response']}`",
        "",
        "## Boundary Status",
        "",
