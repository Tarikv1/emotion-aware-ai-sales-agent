#!/usr/bin/env python3
from __future__ import annotations

import json
import re
import subprocess
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
RUNNER = ROOT / "scripts" / "run_voice_022_spoken_text_normalization.py"
CASES_PATH = ROOT / "research" / "experiments" / "cases" / "voice-022-spoken-text-normalization.json"
TMP_DIR = ROOT / ".tmp" / "voice-022-validation"
TMP_JSON = TMP_DIR / "VOICE-022-spoken-text-normalization.json"
TMP_REPORT = TMP_DIR / "VOICE-022-spoken-text-normalization-report.md"

SECRET_PATTERN = re.compile(
    r"(sk_car_[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9_-]{20,}|AIza[0-9A-Za-z_-]{20,}|xox[baprs]-[A-Za-z0-9-]{20,}|CARTESIA_API_KEY\s*=\s*[^\s]+|ELEVENLABS_API_KEY\s*=\s*[^\s]+|OPENAI_API_KEY\s*=\s*[^\s]+|Authorization:\s*Bearer\s+[A-Za-z0-9]|X-API-Key\s*[:=]\s*[A-Za-z0-9]|xi-api-key\s*[:=]\s*[A-Za-z0-9])"
)


def assert_condition(condition: bool, message: str) -> None:
    if not condition:
        raise AssertionError(message)


def run_command(args: list[str]) -> subprocess.CompletedProcess[str]:
    return subprocess.run(args, cwd=ROOT, text=True, capture_output=True, timeout=30)


def find_segment(packet: dict, segment_id: str) -> dict:
    for segment in packet["output_segments"]:
        if segment["segment_id"] == segment_id:
            return segment
    raise AssertionError(f"Segment not found in output: {segment_id}")


def validate_case(result: dict) -> None:
    packet = result["spoken_text_normalization"]
    expected = result["expected"]
    tts_text = packet["tts_text"]

    assert_condition(packet["validation"]["passed"] is True, f"{result['case_id']} failed packet validation.")
    assert_condition(
        expected["min_normalizations"] <= packet["normalization_count"] <= expected["max_normalizations"],
        f"{result['case_id']} normalization count {packet['normalization_count']} outside expected range {expected}.",
    )

    for fragment in expected.get("required_fragments", []):
        assert_condition(fragment in tts_text, f"{result['case_id']} missing required fragment: {fragment!r}")
    for fragment in expected.get("forbidden_fragments", []):
        assert_condition(fragment not in tts_text, f"{result['case_id']} contains forbidden fragment: {fragment!r}")

    for protected_segment_id in expected.get("protected_segment_ids_unchanged", []):
        segment = find_segment(packet, protected_segment_id)
        assert_condition(
            segment["text_before"] == segment["text_after"],
            f"{result['case_id']} protected segment changed: {protected_segment_id}",
        )
        assert_condition(
            segment["normalizations"] == [],
            f"{result['case_id']} normalized protected segment: {protected_segment_id}",
        )
        assert_condition(
            segment["protected_reason"] is not None,
            f"{result['case_id']} expected protected reason for {protected_segment_id}",
        )

    for segment in packet["output_segments"]:
        if segment["protected_reason"] is not None:
            assert_condition(
                segment["text_before"] == segment["text_after"],
                f"{result['case_id']} changed protected segment {segment['segment_id']}.",
            )
            assert_condition(
                segment["normalizations"] == [],
                f"{result['case_id']} added normalizations to protected segment {segment['segment_id']}.",
            )
        if segment["normalizations"]:
            assert_condition(
                segment["eligible_for_spoken_normalization"] is True,
                f"{result['case_id']} normalized ineligible segment {segment['segment_id']}.",
            )


def main() -> None:
    assert_condition(RUNNER.exists(), "VOICE-022 runner is missing.")
    assert_condition(CASES_PATH.exists(), "VOICE-022 case file is missing.")
    TMP_DIR.mkdir(parents=True, exist_ok=True)

    completed = run_command(
        [
            sys.executable,
            str(RUNNER),
            "--cases",
            str(CASES_PATH),
            "--out",
            str(TMP_JSON),
            "--report-out",
            str(TMP_REPORT),
        ]
    )
    assert_condition(completed.returncode == 0, completed.stderr)
    assert_condition(TMP_JSON.exists(), "VOICE-022 validation JSON was not created.")
    assert_condition(TMP_REPORT.exists(), "VOICE-022 validation report was not created.")

    payload = json.loads(TMP_JSON.read_text(encoding="utf-8"))
    summary = payload["summary"]
    assert_condition(summary["case_count"] == 8, summary)
    assert_condition(summary["languages"]["de"] == 4, summary)
    assert_condition(summary["languages"]["en"] == 4, summary)
    assert_condition(summary["validation_failed"] == 0, summary)
    assert_condition(summary["provider_calls_made"] is False, summary)
    assert_condition(summary["requires_api_key"] is False, summary)
    assert_condition(summary["customer_audio_uploaded"] is False, summary)
    assert_condition(summary["voice_cloning_used"] is False, summary)
    assert_condition(summary["protected_segment_change_count"] == 0, summary)
    assert_condition(summary["normalization_count"] >= 10, summary)

    first_payload_text = TMP_JSON.read_text(encoding="utf-8")
    completed_again = run_command(
        [
            sys.executable,
            str(RUNNER),
            "--cases",
            str(CASES_PATH),
            "--out",
            str(TMP_JSON),
            "--report-out",
            str(TMP_REPORT),
        ]
    )
    assert_condition(completed_again.returncode == 0, completed_again.stderr)
    assert_condition(
        first_payload_text == TMP_JSON.read_text(encoding="utf-8"),
        "VOICE-022 output should be deterministic for the same inputs.",
    )

    payload = json.loads(TMP_JSON.read_text(encoding="utf-8"))
    for result in payload["results"]:
        validate_case(result)

    output_text = json.dumps(payload, ensure_ascii=False) + TMP_REPORT.read_text(encoding="utf-8") + completed.stdout
    match = SECRET_PATTERN.search(output_text)
    if match is not None:
        raise AssertionError(f"Potential secret-like value found in VOICE-022 output: {match.group(0)!r}")
    print("VOICE-022 spoken text normalization validation passed.")


if __name__ == "__main__":
    main()
