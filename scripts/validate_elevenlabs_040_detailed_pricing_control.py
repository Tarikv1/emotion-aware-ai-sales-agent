...
PROMPT_MARKERS = (
    ...
    "CRM/portal capability/price/scope/setup/logistics: direct answer only; no contact invention, mockup, or email CTA.",
...
PRICING_KB_MARKERS = (
    ...
    "Self-update editing access is a CMS/editor setup question, not an appointment-request form.",
    "If the buyer then asks whether a live calendar is extra, treat that as price intent and quote only the $4,000-$6,500 whole-project integration range.",
    "When a buyer asks for both simple CRM handoff and direct CRM/API prices, first ask only whether the work is for a new site or an existing site.",
    "domain registration",
...
OUTPUT_MARKERS = (
    ...
    "PRICE OUTPUT LOCK: the response that first answers explicit price intent ends after one range, one driver, and at most one necessary scope question.",
    "Setup, platform, compatibility, access, requested-details, and logistics questions after a quote stay in that same price-only lane.",
    ...
    "Capability answers and CRM comparisons do not reopen the mockup, send, or email path.",
