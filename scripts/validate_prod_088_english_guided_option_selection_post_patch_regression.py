...
CASE_FILE = ROOT / "research" / "experiments" / "cases" / "prod-088-english-guided-option-selection-post-patch-regression.json"

REQUIRED_FILES = {
    "module": ROOT / "scripts" / "prod_088_english_guided_option_selection_post_patch_regression.py",
    "runner": ROOT / "scripts" / "run_prod_088_english_guided_option_selection_post_patch_regression.py",
    "validator": ROOT / "scripts" / "validate_prod_088_english_guided_option_selection_post_patch_regression.py",
    ...
    "doc": ROOT / "docs" / "product" / "PROD_088_ENGLISH_GUIDED_OPTION_SELECTION_POST_PATCH_REGRESSION.md",
...
GENERATED_FILES = {
    ...
    "guided_option_regression_cases": OUT_DIR / "guided_option_regression_cases.json",
...
def validate_case_file() -> None:
    ...
    assert_condition(payload["scope"] == "english_guided_option_selection_post_patch_regression", payload)
...
def validate_payloads() -> None:
    ...
    guided = read_json(GENERATED_FILES["guided_option_regression_cases"])
    ...
    assert_condition(summary["guided_option_positive_failures"] == 0, summary)
    ...
    assert_condition(guided["case_count"] >= 8, guided)
    assert_condition(adjacent["failure_count"] == 0, adjacent)
    assert_condition(adjacent["case_count"] >= 10, adjacent)
...
def validate_docs() -> None:
    ...
    for marker in [
        "prod-088",
        "english guided option selection post-patch regression",
