...
from prod_021_live_shaped_dialogue_policy_simulation import (
    ...
    DEFAULT_CASE_PATH,
    DEFAULT_PROD_011_CASE,
    DEFAULT_PROD_020_RESULT,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str | None, default: Path, *, allow_missing: bool = True) -> Path:
    ...
    try:
        ...
        raise ValueError(f"PROD-021 path must stay inside project root: {path_text}") from exc
    if _contains_private_path_parts(resolved):
        raise ValueError(f"PROD-021 path is restricted: {path_text}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the PROD-021 live-shaped dialogue-policy simulation checkpoint.")
    parser.add_argument("--case-file", default=str(DEFAULT_CASE_PATH), help="PROD-021 live-shaped case JSON path.")
    parser.add_argument("--prod-011-case", default=str(DEFAULT_PROD_011_CASE), help="PROD-011 case JSON path.")
    parser.add_argument("--prod-020-result", default=str(DEFAULT_PROD_020_RESULT), help="PROD-020 result JSON path.")
...
def main() -> None:
    args = parse_args()
    case_file = resolve_path(args.case_file, DEFAULT_CASE_PATH)
    prod_011_case = resolve_path(args.prod_011_case, DEFAULT_PROD_011_CASE, allow_missing=False)
    prod_020_result = resolve_path(args.prod_020_result, DEFAULT_PROD_020_RESULT, allow_missing=False)
    ...
    payload = build_payload(
        case_file,
        prod_011_case_path=prod_011_case,
        prod_020_result_path=prod_020_result,
