"""Validate appointment-readiness residual handling after the audit.

The validator focuses on whether true readiness asks for a next step while
weak impact, friction, language repair, and concrete callback-time capture do
not get mislabeled as missed appointment asks. It uses dry-run turn builders
...
CHECKPOINT_ID = "UNIVERSAL-APPOINTMENT-READINESS-RESIDUALS-001"
...
def has_next_step(text: str) -> bool:
    return contains_any(text, NEXT_STEP_PATTERNS)
...
def has_weak_close(text: str) -> bool:
    return contains_any(text, WEAK_CLOSE_PATTERNS)
...
def common_assertions(packet: dict[str, Any], failures: list[str]) -> None:
    ...
    for phrase in ["calendar invite", "i sent", "i have sent", "i'll send", "i will send", "crm"]:
        if phrase in text:
            add_failure(failures, f"fake side-effect claim: {phrase}")
...
def scenario_turns(label: str, campaign: dict[str, Any]) -> list[str]:
    ...
    return {
        "readiness_should_ask_next_step": ready_path,
        "readiness_send_info_contact_path": [*ready_path, "send me details"],
        "weak_impact_not_flagged": ["__agent_open__", "yeah sure", campaign["pain"], "not really, just annoying"],
        "frustration_not_flagged": ["__agent_open__", "yeah sure", campaign["pain"], "you're annoying"],
        "language_mismatch_not_flagged": ["__agent_open__", "yeah sure", campaign["pain"], "I don't speak English well"],
        "concrete_time_closes_or_captures": [*ready_path, "tomorrow at 3 works"],
    }[label]
...
def evaluate_scenario(label: str, campaign: dict[str, Any], packets: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if label == "readiness_should_ask_next_step":
        ...
        if not has_next_step(text):
            add_failure(failures, "readiness response did not ask callback/time/contact next step")
        if has_weak_close(text):
            add_failure(failures, "weak close language used despite readiness")
        if call_control(packet) != "continue-call":
            add_failure(failures, f"expected continue-call, got {call_control(packet)}")
    elif label == "readiness_send_info_contact_path":
        ...
        if not has_next_step(text):
            add_failure(failures, "send-info after readiness did not ask email/contact/callback path")
    elif label == "weak_impact_not_flagged":
    ...
    elif label == "frustration_not_flagged":
    ...
    elif label == "language_mismatch_not_flagged":
        if current.get("buyer_move_id") != "language_mismatch":
            add_failure(failures, f"expected language_mismatch, got {current.get('buyer_move_id')}")
        if has_next_step(text):
            add_failure(failures, "language mismatch received appointment pressure")
    elif label == "concrete_time_closes_or_captures":
...
def write_evidence(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for result in results:
        by_scenario[result["scenario"]]["passed" if result["status"] == "pass" else "failed"] += 1
        for failure in result["failures"]:
            failure_types[failure] += 1
        warning_counts.update(result.get("mechanical_warning_flags") or [])
...
def main() -> None:
    scenarios = [
        "readiness_should_ask_next_step",
        "readiness_send_info_contact_path",
        "weak_impact_not_flagged",
        "frustration_not_flagged",
        "language_mismatch_not_flagged",
        "concrete_time_closes_or_captures",
    ]
