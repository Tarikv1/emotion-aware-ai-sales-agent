...
def speech_realism_campaign(campaign: dict[str, Any], enabled: bool) -> dict[str, Any]:
    adjusted = deepcopy(campaign)
    profile = dict(adjusted.get("speech_realism", {}))
    profile["enabled"] = enabled
    if not enabled:
        profile["filler_frequency"] = "off"
        profile["max_bundles_per_response"] = 0
        profile["allow_thinking_fillers"] = False
    adjusted["speech_realism"] = profile
    return adjusted
...
def audio_filename(language: str, variant_id: str, script_id: str, extension: str) -> str:
    ...
    return f"VOICE-024-{normalize_language(language)}-{safe_variant}-{safe_script}.{extension}"
...
def voice_settings_for_provider(provider: dict[str, Any], provider_rendering: dict[str, Any]) -> dict[str, Any]:
    ...
    if provider.get("provider_key") == "elevenlabs":
        ...
        settings["speed"] = round(min(1.09, max(1.03, rendered_speed if rendered_speed != 1.0 else base_speed)), 3)
    return settings
...
def validate_pipeline(
    ...
    return {
        "validator": "VOICE-024 A/B voice delivery pipeline check",
        "passed": passed,
        ...
        "notes": (
            "A/B pipeline preserved protected text and rendered provider-safe TTS input."
            if passed
            else "A/B pipeline failed a protected-text or provider-rendering check."
        ),
    }
...
def run_script_variant(
    ...
    enabled = bool(variant["speech_realism_enabled"])
    variant_campaign = speech_realism_campaign(campaign, enabled)
    ...
    prosody = apply_prosody_naturalness(variant_campaign, realistic_segments, language=language, seed=seed)
    provider_result_shape = build_provider_result_shape(script, variant_campaign, prosody, language)
    ...
    validation = validate_pipeline(spoken_text_normalization, speech_realism, prosody, provider_rendering)
    return {
        ...
        "speech_realism_enabled": enabled,
...
def summarize(
    ...
    return {
        ...
        "timeout_seconds": timeout_seconds,
        "with_voice_023_bundle_count": sum(
            result["speech_realism_bundle_count"] for result in results if result["variant_id"] == "with_voice_023"
        ),
        "without_voice_023_bundle_count": sum(
            result["speech_realism_bundle_count"] for result in results if result["variant_id"] == "without_voice_023"
        ),
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- With VOICE-023 bundle count: `{summary['with_voice_023_bundle_count']}`",
        f"- Without VOICE-023 bundle count: `{summary['without_voice_023_bundle_count']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VOICE-024 speech-realism live-capable A/B listening harness.")
    parser.add_argument("--cases", default=str(DEFAULT_CASES), help="VOICE-024 case/config file.")
