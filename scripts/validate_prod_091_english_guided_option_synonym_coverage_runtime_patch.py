...
CASE_FILE = ROOT / "research" / "experiments" / "cases" / "prod-091-english-guided-option-synonym-coverage-runtime-patch.json"
RUNTIME_FILE = ROOT / "runtime" / "core" / "realtime_turns.py"

REQUIRED_FILES = {
    "module": ROOT / "scripts" / "prod_091_english_guided_option_synonym_coverage_runtime_patch.py",
    "runner": ROOT / "scripts" / "run_prod_091_english_guided_option_synonym_coverage_runtime_patch.py",
    "validator": ROOT / "scripts" / "validate_prod_091_english_guided_option_synonym_coverage_runtime_patch.py",
    "source_validator": ROOT / "scripts" / "validate_prod_090_english_guided_option_synonym_coverage_narrow_probe.py",
    "doc": ROOT / "docs" / "product" / "PROD_091_ENGLISH_GUIDED_OPTION_SYNONYM_COVERAGE_RUNTIME_PATCH.md",
...
def validate_sources() -> None:
    ...
    assert_condition(result["summary"]["current_runtime_gap_count"] == 2, result["summary"])
...
def validate_case_file() -> None:
    ...
    assert_condition(payload["scope"] == "english_guided_option_synonym_coverage_runtime_patch", payload)
...
def validate_payloads() -> None:
    ...
    assert_condition(summary["selected_gap_fixed_count"] == 2, summary)
    assert_condition(summary["positive_case_failures"] == 0, summary)
    assert_condition(summary["control_case_failures"] == 0, summary)
    assert_condition(summary["current_runtime_gap_count_before_patch"] == 2, summary)
    ...
    assert_condition(patch["provider_side_by_side_guard_added"] is True, patch)
    assert_condition(patch["selected_gap_case_ids"] == ["prod-081-guided-option-02", "prod-081-plan-difference-02"], patch)
    assert_condition(positives["failure_count"] == 0, positives)
    assert_condition(positives["case_count"] >= 4, positives)
    assert_condition(controls["failure_count"] == 0, controls)
    assert_condition(controls["case_count"] >= 9, controls)
...
def validate_docs() -> None:
    ...
    for marker in [
        "prod-091",
        "english guided option synonym coverage runtime patch",
