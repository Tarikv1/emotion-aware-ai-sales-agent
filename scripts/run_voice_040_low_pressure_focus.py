...
VOICE_LOW_PRESSURE_FOCUS_RUNTIME_ID = "VOICE-040-low-pressure-focus"
DEFAULT_CASES = ROOT / "research" / "experiments" / "cases" / "voice-040-low-pressure-focus.json"
DEFAULT_RUN_DIR = ROOT / "research" / "experiments" / "generated" / "VOICE-040-low-pressure-focus"
...
def expectation_met(expected: dict[str, Any], key: str, actual: bool) -> bool:
    if key not in expected:
        return True
    return actual is bool(expected[key])
...
def validate_case(case: dict[str, Any], packet: dict[str, Any]) -> dict[str, Any]:
    ...
    low_pressure = voice_delivery["voice_low_pressure_focus"]
    tts = packet["tts_delivery"]
    minimum_chars = int(expected.get("minimum_tts_chars", 0))
    protected_ok = not expected.get("protected_text_locked") or low_pressure["rewrite_count"] == 0
    german_ok = not expected.get("german_text_locked") or low_pressure["language"] == "de" and low_pressure["rewrite_count"] == 0
    source_ok = True if not expected.get("tts_input_source") else tts["tts_input_source"] == expected["tts_input_source"]
    runtime_ok = (
        voice_delivery["validation"]["passed"] is True
        and semantic["validation"]["passed"] is True
        and low_pressure["validation"]["passed"] is True
    ...
    passed = all(
        [
            expectation_met(expected, "semantic_rewrite_applied", semantic["rewrite_count"] > 0),
            expectation_met(expected, "low_pressure_rewrite_applied", low_pressure["rewrite_count"] > 0),
            voice_delivery["final_response_unchanged"] is True,
            source_ok,
            len(tts["tts_input_text"]) >= minimum_chars,
            protected_ok,
            german_ok,
            runtime_ok,
        ]
    ...
    return {
        "passed": passed,
        "semantic_expectation_met": expectation_met(expected, "semantic_rewrite_applied", semantic["rewrite_count"] > 0),
        "low_pressure_expectation_met": expectation_met(expected, "low_pressure_rewrite_applied", low_pressure["rewrite_count"] > 0),
        "tts_input_source_met": source_ok,
        "minimum_tts_chars_met": len(tts["tts_input_text"]) >= minimum_chars,
...
def run_case(
    ...
    *,
    ...
    packet = attach_runtime_tts_delivery(
        ...
        command_name="scripts/run_voice_040_low_pressure_focus.py",
    )
    ...
    return {
        ...
        "voice_semantic_emphasis": packet["voice_delivery"]["voice_semantic_emphasis"],
        "voice_low_pressure_focus": packet["voice_delivery"]["voice_low_pressure_focus"],
        "provider_rendering": {
            ...
            "low_pressure_focus_applied": provider_rendering.get("low_pressure_focus_applied", False),
        ...
        "long_script_tts_chars": len(packet["tts_delivery"]["tts_input_text"]),
...
def summarize(results: list[dict[str, Any]], *, live: bool, timeout_seconds: float) -> dict[str, Any]:
    return {
        ...
        "low_pressure_rewrite_count": sum(result["voice_low_pressure_focus"]["rewrite_count"] for result in results),
        "protected_rewrite_count": sum(
            result["voice_low_pressure_focus"]["rewrite_count"]
            for result in results
            if result["voice_low_pressure_focus"]["protected_segment_count"] > 0
        ),
        "final_response_change_count": sum(
            0 if result["voice_low_pressure_focus"]["validation"]["plain_text_changed"] is False else 1
            for result in results
        ),
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Low-pressure rewrites: `{summary['low_pressure_rewrite_count']}`",
    ...
    for result in payload["results"]:
        low_pressure = result["voice_low_pressure_focus"]
        ...
        lines.extend(
            [
                ...
                f"- Low-pressure rewrites: `{low_pressure['rewrite_count']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run VOICE-040 low-pressure focus through RESP-002/RESP-003.")
    parser.add_argument("--cases", default=str(DEFAULT_CASES), help="VOICE-040 case file.")
...
def main() -> None:
    ...
    if source_cases_path is None:
        raise SystemExit("VOICE-040 source_cases path is required.")
    ...
    payload = {
        "voice_low_pressure_focus_runtime_id": VOICE_LOW_PRESSURE_FOCUS_RUNTIME_ID,
        "title": config["title"],
        "provider": args.provider,
