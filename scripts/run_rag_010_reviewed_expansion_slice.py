...
from rag_reviewed_expansion_slice import build_reviewed_expansion_slice, render_reviewed_expansion_slice_report


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RAG009_RESULT = ROOT / "research" / "experiments" / "generated" / "RAG-009-all-source-review-coverage" / "result.json"
DEFAULT_CASE = ROOT / "research" / "experiments" / "cases" / "rag-010-reviewed-expansion-slice.json"
DEFAULT_OUTPUT_DIR = ROOT / "research" / "experiments" / "generated" / "RAG-010-reviewed-expansion-slice"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the RAG-010 reviewed expansion slice.")
    parser.add_argument("--rag009-result", default=str(DEFAULT_RAG009_RESULT), help="RAG-009 coverage result JSON path.")
    parser.add_argument("--case", default=str(DEFAULT_CASE), help="RAG-010 case/config JSON path.")
    parser.add_argument("--out", default=str(DEFAULT_RESULT), help="Output JSON reviewed expansion path.")
    parser.add_argument("--report-out", default=str(DEFAULT_REPORT), help="Output Markdown reviewed expansion report path.")
...
def main() -> None:
    ...
    payload = build_reviewed_expansion_slice(rag009_result, case_path, root=ROOT)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    report_path.write_text(render_reviewed_expansion_slice_report(payload), encoding="utf-8")
