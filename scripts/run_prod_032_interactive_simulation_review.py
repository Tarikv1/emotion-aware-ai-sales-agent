...
from prod_032_interactive_simulation_review import (
    DEFAULT_PACKET,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str, *, must_stay_in_root: bool = True, block_private: bool = True) -> Path:
    ...
    if must_stay_in_root:
        try:
            ...
            raise ValueError(f"PROD-032 path must stay inside project root: {path_text}") from exc
    if block_private and any(part.lower() in RESTRICTED_PARTS for part in resolved.parts):
        raise ValueError(f"PROD-032 path is restricted: {path_text}")
    return resolved


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PROD-032 interactive simulation review checkpoint.")
    parser.add_argument("--source-result", default=str(DEFAULT_SOURCE_RESULT), help="PROD-031 result JSON path.")
    parser.add_argument("--source-trace", default=str(DEFAULT_SOURCE_TRACE), help="PROD-031 interactive trace JSON path.")
