...
RAG_SALES_COMMUNICATION_SOURCE_EXPANSION_ID = "RAG-019-sales-communication-source-expansion"
PRIVATE_PATH_PARTS = (("data", "private"), ("data", "private-restricted"))
REQUIRED_TOPIC_COUNT = 15
...
def resolve_project_path(path_value: Path | str, root: Path) -> Path:
    ...
    try:
        ...
        raise ValueError(f"RAG-019 path must stay inside project root: {path_value}") from exc
    if _contains_private_path_parts(resolved):
        raise ValueError(f"RAG-019 path is restricted: {path_value}")
    return resolved
...
def _assert_runtime_boundaries_disabled(payload: dict[str, Any]) -> None:
    ...
    for key in (
        ...
        if boundaries.get(key) is True or payload.get(key) is True:
            raise ValueError(f"RAG-019 enables forbidden boundary: {key}")


def _source_map(payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    sources = payload.get("source_registry", [])
    if not isinstance(sources, list) or not sources:
        raise ValueError("RAG-019 source_registry is required.")
    by_id: dict[str, dict[str, Any]] = {}
    for source in sources:
        source_id = str(source.get("source_id", "")).strip()
        if not source_id:
            raise ValueError("RAG-019 source is missing source_id.")
        if source_id in by_id:
            raise ValueError(f"RAG-019 duplicate source_id: {source_id}")
        if not str(source.get("url", "")).startswith("https://"):
            raise ValueError(f"RAG-019 source must have https URL: {source_id}")
...
def _normalize_item(item: dict[str, Any], sources_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if not knowledge_id or not chunk_id or not lane or not source_ids:
        raise ValueError(f"RAG-019 item missing required fields: {knowledge_id or item}")
    missing_sources = [source_id for source_id in source_ids if source_id not in sources_by_id]
    if missing_sources:
        raise ValueError(f"RAG-019 item references unknown sources: {knowledge_id} {missing_sources}")
    if "source_excerpt" in json.dumps(item, sort_keys=True).lower():
        raise ValueError(f"RAG-019 item stores forbidden source excerpt text: {knowledge_id}")
...
def build_sales_communication_source_expansion(case_path: Path | str, *, root: Path | None = None) -> dict[str, Any]:
    ...
    if case_payload.get("sales_communication_source_expansion_id") != RAG_SALES_COMMUNICATION_SOURCE_EXPANSION_ID:
        raise ValueError("RAG-019 case ID mismatch.")
    ...
    if not items:
        raise ValueError("RAG-019 requires at least one knowledge item.")
    ...
    if duplicates:
        raise ValueError(f"RAG-019 duplicate knowledge IDs: {duplicates}")

    covered_topics = [str(topic) for topic in case_payload.get("covered_requested_topics", [])]
    if len(set(covered_topics)) < REQUIRED_TOPIC_COUNT:
        raise ValueError("RAG-019 does not cover all requested topic groups.")

    lane_counts = Counter(item["lane"] for item in items)
    summary = {
        "source_count": len(sources_by_id),
        "knowledge_item_count": len(items),
        "covered_requested_topic_count": len(set(covered_topics)),
    ...
    return {
        "sales_communication_source_expansion_id": RAG_SALES_COMMUNICATION_SOURCE_EXPANSION_ID,
        "inputs": {
            "case_path": rel_path(resolved_case_path, root_path),
        },
        "acceptance": case_payload["acceptance"],
        "summary": summary,
        "covered_requested_topics": covered_topics,
...
def render_sales_communication_source_expansion_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Requested topic groups covered: `{summary['covered_requested_topic_count']}`",
    ...
    for topic in payload["covered_requested_topics"]:
