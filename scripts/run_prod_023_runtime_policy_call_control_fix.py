...
from prod_023_runtime_policy_call_control_fix import (
...
def resolve_path(path_text: str | None, default: Path, *, allow_missing: bool = True) -> Path:
    ...
    try:
        ...
        raise ValueError(f"PROD-023 path must stay inside project root: {path_text}") from exc
    if _contains_private_path_parts(resolved):
        raise ValueError(f"PROD-023 path is restricted: {path_text}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the PROD-023 runtime-policy and call-control fix checkpoint.")
    parser.add_argument("--source-gap-packet", default=str(DEFAULT_SOURCE_GAP_PACKET), help="PROD-022 gap packet JSON path.")
...
def main() -> None:
    args = parse_args()
    source_gap_packet = resolve_path(args.source_gap_packet, DEFAULT_SOURCE_GAP_PACKET, allow_missing=False)
