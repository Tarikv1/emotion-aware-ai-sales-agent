...
from rag_buyer_trust_conversation_repair import (
    build_buyer_trust_conversation_repair,
    render_buyer_trust_conversation_repair_report,
)


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CASE = ROOT / "research" / "experiments" / "cases" / "rag-021-buyer-trust-conversation-repair.json"
DEFAULT_OUTPUT_DIR = ROOT / "research" / "experiments" / "generated" / "RAG-021-buyer-trust-conversation-repair"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the RAG-021 buyer trust and conversation-repair pack.")
    parser.add_argument("--case", default=str(DEFAULT_CASE), help="RAG-021 case/config JSON path.")
...
def main() -> None:
    ...
    payload = build_buyer_trust_conversation_repair(case_path, root=ROOT)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    report_path.write_text(render_buyer_trust_conversation_repair_report(payload), encoding="utf-8")
