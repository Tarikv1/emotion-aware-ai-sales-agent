...
from prod_040_callcenteren_conditional_customer_simulation import (
...
def resolve_path(path_text: str, *, must_stay_in_root: bool = True, block_private: bool = True) -> Path:
    ...
    if must_stay_in_root:
        try:
            ...
            raise ValueError(f"PROD-040 path must stay inside project root: {path_text}") from exc
    if block_private and any(part.lower() in RESTRICTED_PARTS for part in resolved.parts):
        raise ValueError(f"PROD-040 path is restricted: {path_text}")
    return resolved


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PROD-040 CallCenterEN conditional customer simulation checkpoint.")
    parser.add_argument("--source-trace", default=str(DEFAULT_SOURCE_TRACE), help="PROD-039 hardened trace path.")
    ...
    parser.add_argument("--trace-out", default=str(DEFAULT_TRACE), help="Conditional trace JSON output path.")
