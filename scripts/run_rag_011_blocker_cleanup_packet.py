...
from rag_blocker_cleanup_packet import build_blocker_cleanup_packet, render_blocker_cleanup_packet_report
...
DEFAULT_CASE = ROOT / "research" / "experiments" / "cases" / "rag-011-blocker-cleanup-packet.json"
DEFAULT_OUTPUT_DIR = ROOT / "research" / "experiments" / "generated" / "RAG-011-blocker-cleanup-packet"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the RAG-011 blocker cleanup packet.")
    ...
    parser.add_argument("--case", default=str(DEFAULT_CASE), help="RAG-011 case/config JSON path.")
    parser.add_argument("--out", default=str(DEFAULT_RESULT), help="Output JSON cleanup packet path.")
...
def main() -> None:
    ...
    payload = build_blocker_cleanup_packet(rag009_result, rag006_packet, case_path, root=ROOT)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    report_path.write_text(render_blocker_cleanup_packet_report(payload), encoding="utf-8")
