...
from rag_source_mapping_batches import build_source_mapping_batches, render_source_mapping_batches_report


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RAG014_RESULT = (
    ROOT / "research" / "experiments" / "generated" / "RAG-014-source-mapped-quote-followup" / "result.json"
)
...
DEFAULT_CASE = ROOT / "research" / "experiments" / "cases" / "rag-015-source-mapping-batches.json"
DEFAULT_OUTPUT_DIR = ROOT / "research" / "experiments" / "generated" / "RAG-015-source-mapping-batches"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the RAG-015 source-mapping batch packet.")
    parser.add_argument("--rag014-result", default=str(DEFAULT_RAG014_RESULT), help="RAG-014 review JSON path.")
    ...
    parser.add_argument("--case", default=str(DEFAULT_CASE), help="RAG-015 case/config JSON path.")
...
def main() -> None:
    args = parse_args()
    rag014_result = resolve_path(args.rag014_result)
    ...
    payload = build_source_mapping_batches(rag014_result, rag013_result, rag006_packet, rag009_result, case_path, root=ROOT)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    report_path.write_text(render_source_mapping_batches_report(payload), encoding="utf-8")
