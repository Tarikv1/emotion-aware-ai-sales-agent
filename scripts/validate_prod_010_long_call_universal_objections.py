...
RUNNER = ROOT / "scripts" / "run_prod_010_long_call_universal_objections.py"
CASE_PATH = ROOT / "research" / "experiments" / "cases" / "prod-010-long-call-universal-objections.json"
DOC_PATH = ROOT / "docs" / "product" / "PROD_010_LONG_CALL_UNIVERSAL_OBJECTIONS.md"
RESULT_PATH = ROOT / "research" / "experiments" / "generated" / "PROD-010-long-call-universal-objections" / "result.json"
REPORT_PATH = ROOT / "research" / "experiments" / "generated" / "PROD-010-long-call-universal-objections" / "report.md"
COMMANDS = ROOT / "docs" / "product" / "COMMANDS.md"

EXPECTED_ID = "PROD-010-long-call-universal-objections"
REQUIRED_LAYERS = {
    "buyer_state",
    "strategy",
    "safety",
...
def validate_payload(payload: dict[str, Any], report: str) -> None:
    assert_condition(payload["prod_010_id"] == EXPECTED_ID, payload)
    assert_condition(payload["source_checkpoint"] == "PROD-009-cross-domain-generated-gauntlet", payload)
    assert_condition(payload["generator_id"] == "brain_002_long_call_objection_generator", payload)
    protocol = payload["experiment_protocol"]
    assert_condition(protocol["fixed_cases"] is True, protocol)
    assert_condition(protocol["long_call_stress"] is True, protocol)
    assert_condition(protocol["universal_objection_stress"] is True, protocol)
    ...
    assert_condition(protocol["editable_surface"] == "long_call_universal_objection_packet_generation", protocol)
    ...
    assert_condition(summary["turn_count"] >= 42, summary)
    assert_condition(summary["average_turns_per_call"] >= 7.0, summary)
    assert_condition(summary["generated_packet_count"] == summary["turn_count"], summary)
    assert_condition(summary["domain_count"] >= 6, summary)
    assert_condition(summary["universal_objection_count"] >= len(REQUIRED_OBJECTIONS), summary)
    assert_condition(summary["eligible_close_count"] >= 2, summary)
    assert_condition(summary["non_sale_call_count"] >= 4, summary)
    ...
    assert_condition(generated["objection_boundary_correctness"] == 1.0, generated)
    assert_condition(generated["long_call_state_continuity"] == 1.0, generated)
    ...
    assert_condition(comparison["decision"].startswith("keep_long_call_objection_packets"), comparison)

    labels = {call["scenario_label"] for call in payload["calls"]}
    objections = {item for call in payload["calls"] for item in call["universal_objections"]}
    assert_condition(REQUIRED_LABELS.issubset(labels), labels)
    assert_condition(REQUIRED_OBJECTIONS.issubset(objections), objections)
    for call in payload["calls"]:
        assert_condition("candidate" not in call, call)
        assert_condition(len(call["turns"]) >= 7, call)
        assert_condition(len(call["universal_objections"]) >= 3, call)
        ...
        for index, generated_turn in enumerate(call["generated_turn_packets"], start=1):
            ...
            assert_condition(packet["buyer_state"]["universal_objections_seen"] == call["universal_objections"], packet)
            assert_condition(packet["buyer_state"]["turn_position"] == index, packet)
            assert_condition(packet["buyer_state"]["turn_count"] == len(call["turns"]), packet)
            ...
            assert_condition(packet["safety"]["objection_boundary_checked"] is True, packet)
    ...
    for marker in [
        "PROD-010",
        "long-call universal objections",
        "BRAIN-002",
        "safe close rate",
        "hard failure rate",
        "non-sale correctness",
        "state packet completeness",
        "objection boundary correctness",
        "long-call state continuity",
...
def main() -> None:
    for path, label in [
        ...
        (RUNNER, "PROD-010 runner"),
        (CASE_PATH, "PROD-010 case file"),
        (DOC_PATH, "PROD-010 product doc"),
    ]:
        assert_condition(path.exists(), f"{label} is missing: {path.relative_to(ROOT)}")

    commands = COMMANDS.read_text(encoding="utf-8")
    assert_condition("run_prod_010_long_call_universal_objections.py" in commands, "PROD-010 runner missing from command map.")
    assert_condition("validate_prod_010_long_call_universal_objections.py" in commands, "PROD-010 validator missing from command map.")
    ...
    print("PROD-010 long-call universal-objection validation passed.")
