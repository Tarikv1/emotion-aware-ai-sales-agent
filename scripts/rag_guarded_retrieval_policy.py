...
def searchable_text(item: dict[str, Any]) -> str:
    return " ".join(
        str(item.get(field, ""))
        for field in ("project_rule", "safe_application", "do_not_use_when", "guardrail_notes")
    )
...
def build_candidate(item: dict[str, Any], query_tokens: set[str], item_tokens: set[str], score: int) -> dict[str, Any]:
    ...
    return {
        ...
        "project_rule": item.get("project_rule", ""),
        "guardrail_notes": item.get("guardrail_notes", ""),
        ...
        "voice_delivery_advisory_only": item.get("lane") == "voice_delivery",
...
def build_guarded_retrieval_policy(
    ...
    *,
    ...
    items = validate_rag007_payload(rag007_payload)
    ...
    return {
        ...
        "inputs": {
            ...
            "rag007_reviewed_slice_id": rag007_payload.get("reviewed_slice_id", ""),
        },
        "summary": summary,
        "policy": {
        ...
        "boundaries": {
            ...
            "reads_data_private": False,
            "only_reviewed_rag007_used": True,
            "runtime_policy_required_before_live_use": True,
        },
    }
...
def render_guarded_retrieval_policy_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Only reviewed RAG-007 used: `{summary['only_reviewed_rag007_used']}`",
        "",
        "## Retrieved Cases",
        "",
