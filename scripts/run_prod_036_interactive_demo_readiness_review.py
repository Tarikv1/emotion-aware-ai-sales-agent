...
from prod_036_interactive_demo_readiness_review import (
    DEFAULT_DEMO_HTML,
    DEFAULT_PACKET,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str, *, must_stay_in_root: bool = True, block_private: bool = True) -> Path:
    ...
    if must_stay_in_root:
        try:
            ...
            raise ValueError(f"PROD-036 path must stay inside project root: {path_text}") from exc
    if block_private and any(part.lower() in RESTRICTED_PARTS for part in resolved.parts):
        raise ValueError(f"PROD-036 path is restricted: {path_text}")
    return resolved


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PROD-036 interactive demo readiness review checkpoint.")
    parser.add_argument("--source-result", default=str(DEFAULT_SOURCE_RESULT), help="PROD-035 result JSON path.")
    parser.add_argument("--source-trace", default=str(DEFAULT_SOURCE_TRACE), help="PROD-035 aligned trace JSON path.")
    ...
    parser.add_argument("--packet-out", default=str(DEFAULT_PACKET), help="Readiness packet JSON output path.")
    parser.add_argument("--html-out", default=str(DEFAULT_DEMO_HTML), help="Static HTML preview output path.")
