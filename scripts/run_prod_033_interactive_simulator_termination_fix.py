...
from prod_033_interactive_simulator_termination_fix import (
    DEFAULT_HTML,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str, *, must_stay_in_root: bool = True, block_private: bool = True) -> Path:
    ...
    if must_stay_in_root:
        try:
            ...
            raise ValueError(f"PROD-033 path must stay inside project root: {path_text}") from exc
    if block_private and any(part.lower() in RESTRICTED_PARTS for part in resolved.parts):
        raise ValueError(f"PROD-033 path is restricted: {path_text}")
    return resolved


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PROD-033 interactive simulator termination fix checkpoint.")
    parser.add_argument("--source-result", default=str(DEFAULT_SOURCE_RESULT), help="PROD-032 result JSON path.")
    ...
    parser.add_argument("--trace-out", default=str(DEFAULT_TRACE), help="Interactive trace JSON output path.")
