...
from prod_020_naturalized_customer_turn_evaluation import (
    DEFAULT_CAMPAIGN_CASE_SOURCE,
    DEFAULT_PROD_015_RESULT,
    DEFAULT_PROD_019_RESULT,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str | None, default: Path, *, allow_missing: bool = True) -> Path:
    ...
    try:
        ...
        raise ValueError(f"PROD-020 path must stay inside project root: {path_text}") from exc
    if _contains_private_path_parts(resolved):
        raise ValueError(f"PROD-020 path is restricted: {path_text}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the PROD-020 naturalized customer-turn evaluation checkpoint.")
    parser.add_argument("--prod-015-result", default=str(DEFAULT_PROD_015_RESULT), help="PROD-015 result JSON path.")
    parser.add_argument("--prod-019-result", default=str(DEFAULT_PROD_019_RESULT), help="PROD-019 result JSON path.")
...
def main() -> None:
    ...
    prod_019_result = resolve_path(args.prod_019_result, DEFAULT_PROD_019_RESULT)
    ...
    payload = build_payload(
        prod_015_result,
        prod_019_result_path=prod_019_result,
