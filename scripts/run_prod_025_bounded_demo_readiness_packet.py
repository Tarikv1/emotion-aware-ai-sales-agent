...
from prod_025_bounded_demo_readiness_packet import (
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str | None, default: Path, *, allow_missing: bool = True) -> Path:
    ...
    try:
        ...
        raise ValueError(f"PROD-025 path must stay inside project root: {path_text}") from exc
    if _contains_private_path_parts(resolved):
        raise ValueError(f"PROD-025 path is restricted: {path_text}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the PROD-025 bounded demo readiness packet checkpoint.")
    parser.add_argument("--source-prod-024-result", default=str(DEFAULT_SOURCE_PROD_024_RESULT), help="PROD-024 result JSON path.")
...
def main() -> None:
    args = parse_args()
    source_prod_024_result = resolve_path(args.source_prod_024_result, DEFAULT_SOURCE_PROD_024_RESULT, allow_missing=False)
    out_path = resolve_path(args.out, DEFAULT_RESULT)
    report_path = resolve_path(args.report_out, DEFAULT_REPORT)

    payload = build_payload(source_prod_024_result)
    write_json(out_path, payload)
    write_text(report_path, render_report(payload))
    print(json.dumps(payload["readiness_summary"], indent=2, ensure_ascii=False))
