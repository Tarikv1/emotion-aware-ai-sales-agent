...
RUNNER_PATH = ROOT / "scripts" / "run_ultravox_warm_session_latency_benchmark_001.py"
RESULT_PATH = ROOT / "research" / "experiments" / "generated" / "ULTRAVOX-WARM-SESSION-LATENCY-001" / "result.json"
REPORT_PATH = ROOT / "research" / "experiments" / "generated" / "ULTRAVOX-WARM-SESSION-LATENCY-001" / "report.md"
...
def assert_required_config(config: dict[str, Any]) -> None:
    expected = {
        "benchmark_id": "ultravox_warm_session_latency_001",
    ...
    required_gates = set(config.get("required_env_gates", []))
    for gate in (
...
def number_or_none(value: Any) -> bool:
...
def main() -> None:
    ...
    assert_no_secret("warm-session config/result/report/runner", json.dumps(config) + json.dumps(result) + report + runner_text)
    ...
    if result.get("evaluation_id") != "ULTRAVOX-WARM-SESSION-LATENCY-001":
        fail("unexpected warm-session latency evaluation_id")
    if result.get("benchmark_id") != config["benchmark_id"]:
        fail("result benchmark_id must match config")
    for key in (
        "no_new_audio_generation",
        "existing_audio_only",
        "no_microphone",
        "no_outbound_phone",
    ):
    ...
    measured_turns = [turn for turn in turns if isinstance(turn, dict) and turn.get("measured_turn") is True]
    if result.get("measured_warm_turn_count") != len([turn for turn in measured_turns if number_or_none(turn.get("user_turn_end_to_first_agent_audio_seconds")) and turn.get("user_turn_end_to_first_agent_audio_seconds") is not None]):
        fail("measured_warm_turn_count must equal measured turns with first-agent-audio latency")
    for turn in measured_turns:
        latency = turn.get("user_turn_end_to_first_agent_audio_seconds")
        if isinstance(latency, (int, float)) and latency <= 0:
            fail("measured warm latency must be greater than zero; zero indicates carryover audio was counted")
    ...
    for turn in turns:
        ...
        for key in (
            ...
            "user_turn_end_to_first_transcript_seconds",
            "user_turn_end_to_first_tool_request_seconds",
            "tool_response_to_first_agent_audio_seconds",
            "user_turn_end_to_first_agent_audio_seconds",
            "agent_audio_chunks",
            "agent_audio_bytes",
    ...
    for line in (
        ...
        if line not in report:
            fail(f"warm-session report missing line: {line}")
    print("ULTRAVOX warm-session latency validation passed.")
