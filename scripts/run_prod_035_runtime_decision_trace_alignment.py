...
from prod_035_runtime_decision_trace_alignment import (
    DEFAULT_HTML,
    DEFAULT_REPORT,
    DEFAULT_RESULT,
...
def resolve_path(path_text: str, *, must_stay_in_root: bool = True, block_private: bool = True) -> Path:
    ...
    if must_stay_in_root:
        try:
            ...
            raise ValueError(f"PROD-035 path must stay inside project root: {path_text}") from exc
    if block_private and any(part.lower() in RESTRICTED_PARTS for part in resolved.parts):
        raise ValueError(f"PROD-035 path is restricted: {path_text}")
    return resolved


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the PROD-035 runtime decision-trace alignment checkpoint.")
    parser.add_argument("--source-result", default=str(DEFAULT_SOURCE_RESULT), help="PROD-034 result JSON path.")
    ...
    parser.add_argument("--trace-out", default=str(DEFAULT_TRACE), help="Aligned trace JSON output path.")
