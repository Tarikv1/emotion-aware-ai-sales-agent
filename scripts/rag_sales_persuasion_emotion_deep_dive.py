...
RAG_SALES_PERSUASION_EMOTION_DEEP_DIVE_ID = "RAG-020-sales-persuasion-emotion-deep-dive"
...
def _assert_runtime_boundaries_disabled(payload: dict[str, Any]) -> None:
    ...
    for key in (
        ...
        if boundaries.get(key) is True or payload.get(key) is True:
            raise ValueError(f"RAG-020 enables forbidden boundary: {key}")


def _source_map(payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    sources = payload.get("source_registry", [])
    if not isinstance(sources, list) or not sources:
        raise ValueError("RAG-020 source_registry is required.")
    by_id: dict[str, dict[str, Any]] = {}
    for source in sources:
        source_id = str(source.get("source_id", "")).strip()
        if not source_id:
            raise ValueError("RAG-020 source is missing source_id.")
        if source_id in by_id:
            raise ValueError(f"RAG-020 duplicate source_id: {source_id}")
        if not str(source.get("url", "")).startswith("https://"):
            raise ValueError(f"RAG-020 source must have https URL: {source_id}")
...
def _normalize_item(item: dict[str, Any], sources_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if not knowledge_id or not chunk_id or not lane or not source_ids:
        raise ValueError(f"RAG-020 item missing required fields: {knowledge_id or item}")
    missing_sources = [source_id for source_id in source_ids if source_id not in sources_by_id]
    if missing_sources:
        raise ValueError(f"RAG-020 item references unknown sources: {knowledge_id} {missing_sources}")
    if "source_excerpt" in json.dumps(item, sort_keys=True).lower():
        raise ValueError(f"RAG-020 item stores forbidden source excerpt text: {knowledge_id}")
...
def build_sales_persuasion_emotion_deep_dive(case_path: Path | str, *, root: Path | None = None) -> dict[str, Any]:
    ...
    if case_payload.get("sales_persuasion_emotion_deep_dive_id") != RAG_SALES_PERSUASION_EMOTION_DEEP_DIVE_ID:
        raise ValueError("RAG-020 case ID mismatch.")
    ...
    if not items:
        raise ValueError("RAG-020 requires at least one knowledge item.")
    ...
    if duplicates:
        raise ValueError(f"RAG-020 duplicate knowledge IDs: {duplicates}")
    ...
    if len(set(covered_topics)) < REQUIRED_TOPIC_COUNT:
        raise ValueError("RAG-020 does not cover all requested deep-dive topic groups.")
    ...
    return {
        "sales_persuasion_emotion_deep_dive_id": RAG_SALES_PERSUASION_EMOTION_DEEP_DIVE_ID,
        "inputs": {
...
def render_sales_persuasion_emotion_deep_dive_report(payload: dict[str, Any]) -> str:
