...
CHECKPOINT_ID = "CONTEXTUAL-BUYER-SEMANTICS-006-send-info-contact-capture"
...
def assert_state(
    ...
    *,
    ...
    state = send_info_state(packet)
    assert_condition(failures, bool(state), f"{label}: send_info_state must be exposed in memory: {snapshot(packet)}")
    if requested is not None:
        assert_condition(failures, state.get("requested") is requested, f"{label}: expected requested={requested}: {snapshot(packet)}")
    if capture_status is not None:
        assert_condition(failures, state.get("capture_status") == capture_status, f"{label}: expected capture_status `{capture_status}`: {snapshot(packet)}")
    if lead_status is not None:
        assert_condition(failures, state.get("lead_status") == lead_status, f"{label}: expected lead_status `{lead_status}`: {snapshot(packet)}")
    assert_condition(failures, state.get("stores_private_contact_in_public_evidence") is False, f"{label}: public evidence flag must stay false: {snapshot(packet)}")
...
def assert_no_fake_appointment(failures: list[str], packet: dict[str, Any], label: str) -> None:
    mem = memory(packet)
...
def validate_send_info_contact_capture(failures: list[str], evidence: dict[str, Any]) -> None:
    scenarios = {
        ...
        "scenario_g_workflow_review_then_send_details_then_yes": ["__agent_open__", "yeah sure", "missed callbacks happen sometimes", "send me details first", "yes send it"],
        "scenario_h_email_plus_callback_time": ["__agent_open__", "yeah sure", "send me details", f"email {RAW_SYNTHETIC_EMAIL} and call tomorrow at 3"],
        "scenario_i_invalid_email_like_text": ["__agent_open__", "yeah sure", "send me details", "send it to alex at example"],
    }
    ...
    assert_state(failures, a3, label="scenario_a_turn3", requested=True, capture_status="needs_email_or_callback_time", lead_status="open_send_info")
    assert_condition(failures, call_control(a3) != "schedule-and-end", f"scenario_a_turn3: send-info request must not schedule-and-end: {snapshot(a3)}")
    ...
    assert_state(failures, b4, label="scenario_b_turn4", capture_status="email_captured", lead_status="open_send_info")
    ...
    assert_condition(failures, RAW_SYNTHETIC_EMAIL not in json.dumps(snapshot(b4)), "scenario_b_turn4: snapshot must redact raw synthetic email")
    assert_no_fake_appointment(failures, b4, "scenario_b_turn4")
    assert_condition(failures, call_control(b4) in {"continue-call", "end-call"}, f"scenario_b_turn4: email-only capture should not schedule appointment/callback: {snapshot(b4)}")
    assert_spoken_contains(failures, b4, {"summary", "callback"}, "scenario_b_turn4")
    ...
    assert_state(failures, c4, label="scenario_c_turn4", capture_status="callback_time_captured", lead_status="open_callback")
    assert_condition(failures, send_info_state(c4).get("captured_callback_time"), f"scenario_c_turn4: expected callback time capture: {snapshot(c4)}")
    assert_spoken_contains(failures, c4, {"confirmed", "callback time"}, "scenario_c_turn4")
    assert_condition(failures, call_control(c4) == "schedule-and-end", f"scenario_c_turn4: callback time should schedule-and-end: {snapshot(c4)}")
    ...
    assert_state(failures, d4, label="scenario_d_turn4", capture_status="needs_email_or_callback_time", lead_status="open_send_info")
    assert_condition(failures, not send_info_state(d4).get("captured_email_redacted"), f"scenario_d_turn4: no email should be captured: {snapshot(d4)}")
    assert_condition(failures, not send_info_state(d4).get("captured_callback_time"), f"scenario_d_turn4: no callback time should be captured: {snapshot(d4)}")
    assert_condition(failures, call_control(d4) != "schedule-and-end", f"scenario_d_turn4: vague yes must not schedule-and-end: {snapshot(d4)}")
    ...
    assert_state(failures, e4, label="scenario_e_turn4", capture_status="refused_contact", lead_status="closed_refused")
    assert_condition(failures, call_control(e4) == "end-call", f"scenario_e_turn4: refusal should end politely: {snapshot(e4)}")
    ...
    assert_state(failures, f4, label="scenario_f_turn4", capture_status="needs_email_or_callback_time", lead_status="open_send_info")
    assert_spoken_contains(failures, f4, {"missed callbacks", "email", "callback time"}, "scenario_f_turn4")
    assert_no_fake_appointment(failures, f4, "scenario_f_turn4")

    g5 = packets_by_scenario["scenario_g_workflow_review_then_send_details_then_yes"][4]
    assert_semantic(failures, g5, label="scenario_g_turn5", expected="send_info_affirmed_without_contact")
    assert_state(failures, g5, label="scenario_g_turn5", capture_status="needs_email_or_callback_time", lead_status="open_send_info")
    assert_no_fake_appointment(failures, g5, "scenario_g_turn5")
    assert_condition(failures, call_control(g5) != "schedule-and-end", f"scenario_g_turn5: vague yes must not fake callback/appointment: {snapshot(g5)}")
    assert_spoken_contains(failures, g5, {"email", "callback time"}, "scenario_g_turn5")

    h4 = packets_by_scenario["scenario_h_email_plus_callback_time"][3]
    assert_semantic(failures, h4, label="scenario_h_turn4", expected={"contact_detail_provided", "email_and_callback_time_captured"})
    assert_state(failures, h4, label="scenario_h_turn4", capture_status="email_and_callback_time_captured", lead_status="open_callback")
    assert_condition(failures, send_info_state(h4).get("captured_email_redacted") == REDACTED_EMAIL, f"scenario_h_turn4: expected redacted email: {snapshot(h4)}")
    assert_condition(failures, send_info_state(h4).get("captured_callback_time"), f"scenario_h_turn4: expected callback time capture: {snapshot(h4)}")
    assert_condition(failures, RAW_SYNTHETIC_EMAIL not in json.dumps(snapshot(h4)), "scenario_h_turn4: snapshot must redact raw synthetic email")
    assert_spoken_contains(failures, h4, {"summary", "callback"}, "scenario_h_turn4")

    i4 = packets_by_scenario["scenario_i_invalid_email_like_text"][3]
    assert_semantic(failures, i4, label="scenario_i_turn4", expected={"contact_detail_unclear", "send_info_affirmed_without_contact"})
    assert_state(failures, i4, label="scenario_i_turn4", capture_status="needs_email_or_callback_time", lead_status="open_send_info")
    assert_condition(failures, not send_info_state(i4).get("captured_email_redacted"), f"scenario_i_turn4: invalid email-like text must not capture email: {snapshot(i4)}")
    assert_spoken_contains(failures, i4, {"repeat the email", "callback time"}, "scenario_i_turn4")
...
def main() -> None:
    ...
    validate_send_info_contact_capture(failures, evidence)
    payload = sanitize(
        {
